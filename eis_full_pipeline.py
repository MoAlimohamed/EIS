...
def model_to_bytes(model):
    # 1. Create a temporary file with the required .h5 extension
    with tempfile.NamedTemporaryFile(suffix=".h5", delete=False) as tmp:
        tmp_name = tmp.name
        
    # 2. Save the model to this file path
    model.save(tmp_name)
    
    # 3. Read the saved file into bytes
...
def model_from_bytes(model_bytes):
    ...
    with tempfile.NamedTemporaryFile(suffix=".h5", delete=False) as tmp:
        tmp_name = tmp.name
        # Write the raw bytes into this temporary file
        tmp.write(model_bytes)
        
    # 2. Let Keras load the model from the physical file (skip compiling to avoid metric errors)
    model = tf.keras.models.load_model(tmp_name, compile=False)
